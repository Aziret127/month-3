...
# Определяем полный текст
full_text = "Это пример текста с разными стилями для консоли Python."

# Определяем спаны (диапазоны текста) и применяем стили
# Span(начальный_индекс, конечный_индекс, "название_стиля")
text_with_spans = Text(
    full_text,
    spans=[
        Span(4, 10, "bold magenta"), # "пример" будет жирным и пурпурным
        Span(18, 25, "underline cyan"), # "разными" будет подчеркнутым и голубым
        Span(26, 33, "bold red reverse") # "стилями" будет жирным, красным с инверсией цвета
    ]
)

# Выводим стилизованный текст в консоль
console.print(text_with_spans)